...
func (r *UserRepo) CountDemo(ctx context.Context) (int, error) {
	var count int
	err := r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM users WHERE is_demo = 1`).Scan(&count)
	if err != nil {
...
func (r *UserRepo) CountAdmins(ctx context.Context) (int, error) {
	var count int
	err := r.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM users WHERE is_admin = 1 AND is_disabled = 0`).Scan(&count)
	if err != nil {
