...
func (r *UserRepo) FindByID(ctx context.Context, id uuid.UUID) (*entities.User, error) {
	row := r.db.QueryRowContext(ctx, `SELECT id, email, password_hash, is_admin, is_disabled, created_at, updated_at FROM users WHERE id = ?`, id.String())
...
func (r *UserRepo) FindByEmail(ctx context.Context, email string) (*entities.User, error) {
	row := r.db.QueryRowContext(ctx, `SELECT id, email, password_hash, is_admin, is_disabled, created_at, updated_at FROM users WHERE email = ?`, email)
	u, err := scanUserRow(row)
	if err == sql.ErrNoRows {
		return nil, nil
	}
...
func (r *UserRepo) Create(ctx context.Context, u *entities.User) error {
	_, err := r.db.ExecContext(ctx, `INSERT INTO users (id, email, password_hash, is_admin, is_disabled, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		u.ID.String(), u.Email, u.PasswordHash, boolToInt(u.IsAdmin), boolToInt(u.IsDisabled), u.CreatedAt.Format(time.RFC3339), u.UpdatedAt.Format(time.RFC3339))
	if err != nil {
...
func (r *UserRepo) Update(ctx context.Context, u *entities.User) error {
	...
	_, err := r.db.ExecContext(ctx, `UPDATE users SET email = ?, password_hash = ?, is_admin = ?, is_disabled = ?, updated_at = ? WHERE id = ?`,
		u.Email, u.PasswordHash, boolToInt(u.IsAdmin), boolToInt(u.IsDisabled), u.UpdatedAt.Format(time.RFC3339), u.ID.String())
	if err != nil {
...
func scanUserFromRow(s scanner) (*entities.User, error) {
	...
	if err := s.Scan(&idStr, &u.Email, &u.PasswordHash, &isAdmin, &isDisabled, &createdAt, &updatedAt); err != nil {
		return nil, err
	}
